...
// Implamentacio d'un arbre k-dimensional binari on cada node té les coordenades k-dimensionals
class KdTree {
...
public:
    ...
    // Constructora: crea un k-d tree buit
    KdTree () {
        primer_node = NULL;
    }
    ...
    void insertRec(Node* & current, vector<double> & coords_node, int discrim) {       
        ...
        else if (current->coordenades[discrim] >= coords_node[discrim]) {     
            insertRec(current->left, coords_node, (discrim+1) % k); 
        } 
        else insertRec(current->right, coords_node, (discrim+1) % k); 
    }

    // Inserir un nou node amb les coordenades k-dimensionals 
    void insert(vector<double> & coords_node) { 
        insertRec(primer_node, coords_node, 0); // el primer discriminant es 0
    }
    ...
    KdTree left() const {
        assert(not empty());
        return KdTree(primer_node->left);
    }

    // Retorna el subarbre dret
    KdTree right() const {
        assert(not empty());
        return KdTree(primer_node->right);
    }
